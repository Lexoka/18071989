...
\cleardoublepage
	...
	\begin{figure}[H]
		\begin{subfigure}[t]{.3\textwidth}
			\centering
			\includegraphics[height=6cm]{./figures/ch1/collagen}
		...
		\begin{subfigure}[t]{.7\textwidth}
			\centering
			\includegraphics[height=6cm]{./figures/ch1/eralpha}
	...
	\paragraph{Sphères de van der Waals.} La représentation en sphères de van der Waals repose sur la notion de rayon de van der Waals. Celle-ci découle elle même de la force de van der Waals. Il s'agit en fait de l'effet combiné de plusieurs forces :
	...
	La représentation en structures secondaires est très courante, notamment pour visualiser des trajectoires de dynamique moléculaire. Cette raison suffit à en faire une application majeure pour une technique de sélection de cibles mobiles. Le niveau d'occultation est particulièrement faible (relativement aux autres représentations, mais il reste élevé dans l'absolu) et les cibles, assez grosses, bougent relativement peu.
	...
	\begin{figure}[H]
		...
		\end{subfigure}
		\caption{Désoxyribonucléase (ou ADNase). Cette enzyme est ici illustrée par diverses représentations surfaciques. Illustrations produites par \emph{UnityMol}~\cite{doutreligne2014unitymol} et \emph{Molekel}~\cite{portmann2000molekel, varetto2009molekel}.}
		\label{fig:4awn_surf}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\end{subfigure}
		\caption[Octaèdres pour les conditions périodiques aux limites.]{Utilisation d'octaèdres pour la mise en œuvre de conditions périodiques aux limites en minimisant les erreurs et les calculs nécessaires. Source : S.R. Saunders, \emph{Real Wireless}\footnotemark.}
		\label{fig:octa}
	\end{figure}
	...
	D'autres paramètres doivent être initialisés, par exemple les vitesses des particules, qui suivent généralement une distribution de Maxwell-Boltzmann~\cite{maxwell1860v, maxwell1860ii, boltzmann1970weitere, boltzmann2003further}. Il convient également d'ajuster la température, la pression, le nombre de particules, etc., et de déterminer lesquels de ces paramètres doivent être maintenus constants, et comment. Le changement d'un seul de ces paramètres mènera généralement à une autre exploration de l'espace conformationnel, c'est-à-dire à une trajectoire\footnote{expliquer ce que c'est} différente.
	
	\paragraph{Champs de force.}
	\emph{CHARMM}~\cite{brooks1983charmm, brooks2009charmm}, \emph{AMBER}~\cite{cornell1995second, wang2004development}, OPLS~\cite{jorgensen1996development, kaminski2001evaluation}, \emph{GROMOS}~\cite{scott1999gromos, oostenbrink2004biomolecular}.
