...
\section{Introduction}
	...
	Paul M. Fitts a donc pu déduire de cette étude la loi qui porte son nom, et que l'on peut décrire par l'équation~\ref{eq:fitts} (d'autres formulations à peu près équivalentes existent).
	
	\begin{equation}
		\label{eq:fitts}
		MT = a + b \times ID = a + b \times \log\left(\frac{A}{W} + 1\right)
	\end{equation}
	
	Ici, $MT$ représente le temps de mouvement, $ID$ représente l'indice de difficulté (développé à droite), $A$ représente l'amplitude, c'est-à-dire la distance entre la cible et le point de départ du mouvement, tandis que les termes $a$ et $b$ sont des constantes déterminées empiriquement, propres à chaque système. La partie la plus intéressante de cette équation est l'indice de difficulté, $\log(2A/W)$. C'est la seule partie variable, et c'est lui qui, comme son nom l'indique, détermine la difficulté de la tâche. Il est aisé à comprendre intuitivement : plus l'amplitude du mouvement nécessaire est grande, plus la tâche sera difficile ; plus la largeur de la cible est grande, plus la tâche sera facile.
	
	L'étude de Paul M. Fitts portait sur des mouvements sur une seule dimension, mais sa loi s'étend aisément au plan~\cite{mackenzie1992extending}, en tenant compte de la forme de la cible~\cite{accot2003refining}, et même à l'espace tridimensionnel~\cite{murata2001extending}. On peut également généraliser la loi de Fitts pour modéliser des tâches de tracé de trajectoires~\cite{accot1997beyond, accot1999performance}.
	Bien que la loi de Fitts soit d'une élégante simplicité et d'une utilité évidente pour 
