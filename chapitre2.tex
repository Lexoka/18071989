...
\section{\emph{Ray casting} avec disambiguïsation}
	...
	\begin{figure}[H]
		...
		\caption[Principe du \emph{Depth Ray}]{Illustration du fonctionnement du \emph{Depth Ray}. Un marqueur rose est utilisé pour sélectionner la cible intersectée la plus proche de lui (a) --- notons que le marqueur fonctionne par proximité et n'a pas besoin d'être \emph{sur} la cible. Reculer le périphérique de pointage recule le marqueur et permet de séletionner une cible plus proche (b). Inversement, avancer le périphérique permet d'avancer le marqueur et de sélectionner une cible plus lointaine (c). Crédit : \cite{grossman2006design}.}
		\label{fig:depthRay}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\caption[Principe du \emph{Lock Ray}]{Illustration du fonctionnement du \emph{Lock Ray}. La sélection commence par une phase de \emph{raycasting} classique, sans marqueur de profondeur (a). Une fois que l'utilisateur a pressé le bouton du périphérique un marqueur de profondeur apparaît au milieu du segmet du rayon inclus dans la sphère (b). L'utilisateur peut déplacer le marqueur en déplaçant sa main, comme dans le \emph{Depth Ray}, et effectue la sélection en relâchant le bouton (c). Crédit : \cite{grossman2006design}.}
		\label{fig:lockRay}
	\end{figure}
	...
	\paragraph{Principe et avantages.}
	Comme le \emph{Lock Ray}, le \emph{Flower Ray}~\cite{grossman2006design} fonctionne avec deux phases distinctes. Dans la première, l'utilisateur place son rayon dans la position et l'orientation qu'il désire ; puis, il le fixe, et les objets croisés par le rayon « s'épanouissent » --- tout autour d'un curseur ponctuel, comme les pétales d'une fleur autour de son pistil, ainsi que l'illustre la figure~\ref{fig:flowerRay}.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figures/ch2/flowerRay}
		\caption[Principe du \emph{Flower Ray}]{Illustration du fonctionnement du \emph{Depth Ray}. Toutes les cibles croisées par le rayon sont colorées en vert (a). Quand l'utilisateur presse le bouton de son périphérique, ces objets s'épanouissent en un menu de sélection autour d'un curseur poncuel, qui apparaît à cet instant (b). L'utilisateur peut à présent pointer près de la cible et relâcher le bouton pour la sélectionner (c). Crédit : \cite{grossman2006design}.}
		\label{fig:flowerRay}
	\end{figure}
