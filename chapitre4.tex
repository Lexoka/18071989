...
\section{Étude de l'aquisition de cibles mobiles erratiques}
	...
	Remarquons tout de même qu'une trajectoire générée par notre modèle peut, temporairement (sur une ou deux secondes, par exemple) approximer un mouvement autocorrélé. Attendu qu'une tâche de sélection, pour peu qu'elle ne soit pas trop difficile, peut se dérouler sur une durée approximativement comparable, le modèle VFA n'est pas nécessairement inutile pour étudier les performances de sélection des objets autocorrélés. On privilégiera dans ce cas de très fortes valeurs de $F$ et de très faibles valeurs de $A$, par exemple. La figure~\ref{fig:autocorr} fournit deux exemples de telles trajectoires que nous proposons d'appeler pseudo-autocorrélées.
	...
	\begin{figure}[!htb]
		...
		\begin{subfigure}[t]{0.49\textwidth}
			...
			\caption[Mouvement pseudo-autocorrélé A]{Première trajectoire générée.}
			\label{fig:acA}
		\end{subfigure}
		~
		\begin{subfigure}[t]{0.49\textwidth}
			...
			\caption[Mouvement pseudo-autocorrélé B]{Deuxième trajectoire générée.}
			\label{fig:acB}
		\end{subfigure}
		\caption{Trajectoires générées avec les mêmes valeurs $F = 120$~Hz et $A = 1\degree$. Ces trajectoires ressemblent à celles que l'on pourrait attendre d'objets autocorrélés, tels que des véhicules, par exemple.}
		\label{fig:autocorr}
	\end{figure}
