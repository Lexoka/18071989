...
\section{Périmètres des enveloppes convexes et temps de sélection}
	...
	\FloatBarrier
	\subsection{Périmètre en fonction de F et A}
	Ainsi, le produit $A\sqrt{F}$ nous permet de déterminer le coefficient $\gamma$ par lequel il faut multiplier la vitesse avant de multiplier le tout par une valeur de référence dépendant de F et de A, afin de trouver le périmètre de l'enveloppe convexe. Nous pouvons donc avancer qu'il existe une fonction $f$ telle que $f\left(A\sqrt{F}\right) = \gamma$. De fait, la formule du périmètre de l'enveloppe convexe que l'on pouvait exprimer selon la forme de l'équation~\ref{eq:areaVFAgamma} peut être réécrite sous la forme de l'équation~\ref{eq:areaVFAonly}, où $\mathcal{P}_{base}(ec)$ est le périmètre \og de base \fg{} de l'enveloppe convexe, que nous choisissons (un peu arbitrairement) de définir comme le périmètre de l'enveloppe convexe d'une trajectoire générée par un couple (F,A) donné avec $V = 1$~cm/s, et divisée par $\gamma{}V$, c'est-à-dire par $\gamma$.

	\begin{align}
		\label{eq:areaVFAgamma}
		\mathcal{P}(ec) &= \mathcal{P}_{base}(ec) \times \gamma{}V \\
		\label{eq:areaVFAonly}
		\mathcal{P}(ec) &= \mathcal{P}_{base}(ec) \times f\left(A\sqrt{F}\right)
	\end{align}
	
	Naturellement, pour pouvoir estimer le périmètre de l'enveloppe convexe à partir de nos paramètres, il faut pouvoir calculer $\mathcal{P}_{base}(ec)$. Nous avons donc calculé le périmètre moyen des enveloppes convexes de trajectoires générées à partir de multiples couples (F,A) avec $V = 1$~cm/s, en générant de nombreuses trajectoires par condition (100) afin d'avoir des valeurs de référence fiables. Dans un premier temps, nous avons calculé les périmètres des enveloppes convexes pour $V = 1$~cm/s, pour de nombreux couples (F,A). La figure~\ref{fig:afAreaLin} représente ces données sous forme de carte thermique, et la figure~\ref{fig:afAreaLog} les représente de la même façon, mais sur une échelle logarithmique, pour plus de lisibilité.
	...
	\begin{figure}[!htb]
		...
		\begin{subfigure}[t]{\textwidth}
			...
			\label{fig:afAreaLog}
		\end{subfigure}
		\caption[Périmètres des enveloppes convexes en fonction de F et A]{Périmètres des enveloppes convexes en fonction de F et A. La fréquence se lit en ordonnée, le paramètre d'angle en abscisse, et la valeur du périmètre est indiquée par la couleur ; elle est comprise entre 0 et 10, et toutes les valeurs indiquées correspondent à une vitesse de 1~cm/s.}
		\label{fig:afArea}
	\end{figure}
	
	La similitude entre ces données et celles indiquant les valeurs de $\gamma$ en fonction du couple (F,A) est particulièrement frappante. Cela laisse supposer que l'influence du produit $\gamma{}V$ pourrait être dominante dans le périmètre de l'enveloppe convexe. Afin de le vérifier, nous avons cherché à exprimer $\mathcal{P}(ec)$ en fonction du produit AF d'une part, ainsi que de $A\sqrt{F}$ d'autre part, comme nous l'avions fait avec $\gamma$ dans la figure~\ref{fig:gammaVentropy}. Les résultats sont présentés sur la figure~\ref{fig:areaVentropy}, et confirment nos impressions.	
	...
	\begin{figure}[!htb]
		...
		\begin{subfigure}[t]{\textwidth}
			...
			\caption{$\mathcal{P}(ec)$ en fonction de $A\times{}F$.}
			\label{fig:af2area}
		\end{subfigure}
		~
		\begin{subfigure}[t]{\textwidth}
			...
			\caption{$\mathcal{P}(ec)$ en fonction de $A\sqrt{F}$, sur une échelle logarithmique.}
			\label{fig:af2areaLogLog}
		\end{subfigure}
		\caption[$\mathcal{P}(ec)$ en fonction de F et A, bis]{$\mathcal{P}(ec)$ en fonction de F et A. Les valeurs de $\mathcal{P}(ec)$ sont en ordonnée, et l'expression contenant A et F est en abscisse. On remarque que la seconde expression fondée sur la racine carrée de F correspond bien mieux aux valeurs de $\mathcal{P}(ec)$.}
		\label{fig:areaVentropy}
	\end{figure}
	
	\subsection{Indépendance du périmètre de base par rapport à F et A}
	Pour rappel, $\mathcal{P}(ec) = \mathcal{P}_{base}(ec) \times \gamma{}V$, donc $\mathcal{P}_{base}(ec) = \frac{\mathcal{P}(ec)}{\gamma{}V}$. Or, nous avons les valeurs de $\mathcal{P}(ec)$ ainsi que celles de $\gamma$, et nous pouvons fixer $V = 1$. Nous avons donc pu calculer les valeurs de $\mathcal{P}_{base}(ec)$ pour tous nos couples (F,A), et avons toujours obtenu une valeur proche de 1,0 : plus précisément, une valeur moyenne de 1,001 avec un écart-type de 0,020.
	
	Nous pouvons donc en déduire que $\mathcal{P}_{base}(ec)$ ne dépend pas de F et A, et que l'influence de ces paramètres sur $\mathcal{P}(ec)$ est entièrement capturée par $\gamma$. Les équations~\ref{eq:areaVFAgamma} et~\ref{eq:areaVFAonly} peuvent donc être réécrites sous la forme de l'équation~\ref{eq:areaGammaV} et ~\ref{eq:areaASQRTF}, respectivement.
	
	\begin{align}
		\label{eq:areaGammaV}
		\mathcal{P}(ec) &= \gamma{}V \\
		\label{eq:areaASQRTF}
		\mathcal{P}(ec) &= f\left(A\sqrt{F}\right)V
	\end{align}
	
	L'équation~\ref{eq:areaASQRTF}, en particulier, nous permet d'estimer l'espérance du périmètre de l'enveloppe convexe d'une trajectoire générée par un triplet (V,F,A) en multipliant V par $f\left(A\sqrt{F}\right)$, où la valeur de $f\left(A\sqrt{F}\right)$ est obtenue à l'aide du modèle que nous avons présenté dans les équations~\ref{eq:brokenPower}, \ref{eq:brokenPowerAF}, et \ref{eq:brokenPowerNum}.
	
\section{Enveloppes convexes et performances de sélection}
	Attendu que le périmètre de l'enveloppe convexe d'une trajectoire correspond à l'espace que la cible peut occuper pendant la durée considérée, il est raisonnable de supposer un lien entre les performances (par exemple le temps de sélection) et $\mathcal{P}(ec)$. La figure~\ref{fig:perfVareaRaw} présente notre exploration de cette relation, avec des valeurs de $\mathcal{P}(ec)$ calculées pour des trajectoires de 2 secondes, comme dans la suite de nos travaux.
	...
	\subsection{Modélisation de la MTSN en fonction de \texorpdfstring{$\mathcal{P}(ec)$}{A(ec)}}
...
\section{Aires des enveloppes convexes et temps de sélection}
	...
	\begin{figure}[!htbp]
		...
		\caption[Coefficients $\delta$ en fonction de F et A]{Carte thermique des coefficients $\delta$ en fonction de F et A, sur une échelle log-log. Les angles se lisent en abscisse, les fréquences en ordonnée, et la valeur de $\delta$ est indiquée par la couleur, selon l'échelle de droite. Le coefficient de corrélation est supérieur à 0,996. Diagonale de difficulté.}
		\label{fig:afRealDelta_log}
	\end{figure}
	
	\subsection{Pseudo-entropie et valeur de \texorpdfstring{$\delta$}{delta}}
	
	Lois de puissance combinées.
	
	Pour modéliser ces données, nous avons opté pour le modèle de l'équation~\ref{eq:doublePower}, avec les valeurs numériques présentées dans l'équation~\ref{eq:doublePowerNum}.
	...
	\begin{figure}[!htbp]
		...
		\caption[Coefficients $\delta$ en fonction de $A\sqrt{F}$]{Coefficients $\delta$ en fonction de $A\sqrt{F}$, sur une échelle log-log. Les deux segments rectilignes sont caractéristiques d'une loi de puissance\ldots{} chacun, d'où l'approximation (en vert).}
	...
	Plus de maths, et heatmaps. Enfin au moins une heatmap.
...
\section{Conclusion}
	...
	Nous nous sommes ensuite intéressés aux périmètres des enveloppes convexes ($\mathcal{P}(ec)$) associées aux trajectoires de nos cibles, en fonction des paramètres de leurs mouvements. Ces périmètres décrivent l'espace qu'une cible est susceptible d'occuper en un temps donné. Nous avons d'abord montré qu'elles sont proportionnelles à la vitesse ; puis, nous avons observé que la pente de cette relation linéaire dépendait des paramètres F et A. Plus précisément, nous avons montré qu'elle dépendait de $A\sqrt{F}$. Mieux, nous avons vu que pour une durée de trajectoire donnée, le périmètre ne dépendait que de la vitesse et d'une fonction de $A\sqrt{F}$, soit d'un coefficient que nous avons appelé $\gamma$. Nous avons de plus proposé un modèle permettant d'estimer précisément l'espérance de $\gamma$, et donc celle de $\mathcal{P}(ec)$ à partir des paramètres V, F et A.
	
	Nous avons montré que les performances de sélection dépendaient de $\mathcal{P}(ec)$, à condition de prendre soin de normaliser d'une part ces périmètres, et d'autre part les temps de sélection par rapport à la vitesse des cibles. On obtient alors une relation non linéaire (et non monotone) entre $\mathcal{P}(ec)$ et les performances de sélection, qu'il s'agisse des temps de sélection ou des taux d'erreurs.
